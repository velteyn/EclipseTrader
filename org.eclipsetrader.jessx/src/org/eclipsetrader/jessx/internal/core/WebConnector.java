   ...
   //TODO EDOZ Qui potrebbe essere inizializzato il client di JESSX con la sua login
    ...
    public boolean sendOrder(OrderMonitor tracker) {
        ...
        try {
            ...
            Status status = new Status(IStatus.ERROR, JessxActivator.PLUGIN_ID, 0, "Error sending order [" + order.toString() + "]", e); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (ok && confirm) {
            ...
            try {
                ...
                Status status = new Status(IStatus.ERROR, JessxActivator.PLUGIN_ID, 0, "Error confirming order [" + order.toString() + "]", e); //$NON-NLS-1$ //$NON-NLS-2$
                JessxActivator.log(status);
            }
        }
    ...
    public boolean cancelOrder(OrderMonitor tracker) {
        ...
        try {
            ...
            Status status = new Status(IStatus.ERROR, JessxActivator.PLUGIN_ID, 0, "Error canceling order [" + tracker.toString() + "]", e); //$NON-NLS-1$ //$NON-NLS-2$
            JessxActivator.log(status);
        }
