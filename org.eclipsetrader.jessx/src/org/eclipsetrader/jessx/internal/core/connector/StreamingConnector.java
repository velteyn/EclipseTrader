    ...
    public void run() {
        ...
        try {
            ...
            if (Util.byteToInt(bHeaderLogin[1]) == CreaMsg.ERROR_MSG) {
                ...
                JessxActivator.log(new Status(IStatus.ERROR, JessxActivator.PLUGIN_ID, 0, "Error connecting to streaming server: " + eMsg.sMessageError, null)); //$NON-NLS-1$
                return;
            }
            try {
                ...
                thread = null;
                JessxActivator.log(new Status(IStatus.ERROR, JessxActivator.PLUGIN_ID, 0, "Error starting data stream", e)); //$NON-NLS-1$
                return;
            }
        } catch (Exception e) {
            JessxActivator.log(new Status(IStatus.ERROR, JessxActivator.PLUGIN_ID, 0, "Error connecting to streaming server", e)); //$NON-NLS-1$
