    ...
    public ISecurity unmarshal(String v) throws Exception {
        ...
        if (JessxActivator.getDefault() != null) {
            try {
                ...
                Status status = new Status(IStatus.ERROR, JessxActivator.PLUGIN_ID, 0, "Error reading repository service", e); //$NON-NLS-1$
                JessxActivator.getDefault().getLog().log(status);
            }
        }

        if (security == null) {
            if (JessxActivator.getDefault() != null) {
                Status status = new Status(IStatus.WARNING, JessxActivator.PLUGIN_ID, 0, "Failed to load security " + uri.toString(), null); //$NON-NLS-1$
                JessxActivator.getDefault().getLog().log(status);
            }
